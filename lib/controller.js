...
Ember.PusherController = Em.Controller.extend({
  ...
  wire: function(target, channelName, events) {
    console.log("wiring: ", target, channelName, events);
    ...
    events.forEach(function(eventName) {
      var handler = function(data) {
        target.send(Em.String.camelize(eventName), data);
      };
  ...
  connectChannel: function(channelName) {
    ...
    if(Em.isEmpty(Object.keys(bindings[channelName].eventBindings))) {
  ...
  channelFor: function(channelName) {
    this.get('bindings')[channelName].channel;
  },
